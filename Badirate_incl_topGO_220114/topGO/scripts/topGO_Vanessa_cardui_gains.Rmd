...
write.table(rbind(allRes_BP[allRes_BP$p_adj<=0.05, ], allRes_CC[allRes_CC$p_adj<=0.05, ], allRes_MF[allRes_MF$p_adj<=0.05, ]), file = "../result_topGO/V_cardui_res_topGO_VC_sign.txt", row.names = FALSE)

#semicolon sep, without quotes
write.table(rbind(allRes_BP[allRes_BP$p_adj<=0.05, ], allRes_CC[allRes_CC$p_adj<=0.05, ], allRes_MF[allRes_MF$p_adj<=0.05, ]), file = "../result_topGO/V_cardui_res_topGO_VC_sign.csv", row.names = FALSE, sep = ";", quote = FALSE)
...
#if not in env
allRes_all_fdr <- read.csv(file = "../result_topGO/V_cardui_res_topGO_VC_sign_corr.csv",header=T, sep = ";")
...
#check genes
#get genes per GO-term
# sel.terms <- allRes_all_fdr$GO.ID
# ann.genes_BP <- genesInTerm(GO_data_BP, sel.terms)
# head(ann.genes_BP)
# 
# 
# ann.genes_CC <- genesInTerm(GO_data_CC, sel.terms)
# head(ann.genes_CC)
# 
# ann.genes_MF <- genesInTerm(GO_data_MF, sel.terms)
# head(ann.genes_MF)
# 
# ann.score <- scoresInTerm(GO_data_MF, sel.terms, use.names = TRUE)
# 
# names(subset(x = geneList, geneList==1))
# subset(x = ann.score$`GO:0055103`, ann.score$`GO:0055103`==2)
