...
sed 's/-/ /' ./../MCScanX/Output/hmel_vca.collinearity | awk 'NR>11 {print $3, $4}' | grep -C 1 "score" |\
			grep -v "score\|--" |\
				awk '{print $1"\n"$2}' \
					> blocklist.tmp

awk 'END {print $3"\n"$4}'  ../../MCScanX/Output/hmel_vca.collinearity >> blocklist.tmp

while read line
do
  grep "$line" ../../MCScanX/Input/hmel_vca.gff
done <blocklist.tmp |\
	paste -d " " - - - - |\
