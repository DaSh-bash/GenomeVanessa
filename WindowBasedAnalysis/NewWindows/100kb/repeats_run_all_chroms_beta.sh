...
do
	...
	do
		...
		nohup bash repeat_per_chromosome_beta_merged.sh $1 $len $line $2 > $line-repeat-count.out &
#		bash repeat_per_chromosome_beta.sh $1 $len $line $2  #> $line-repeat-count.out 		
		#wait
	done
done < "$3" 
