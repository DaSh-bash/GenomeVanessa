...
rec_rate <- read.table("window_based_data/output_marey_map_all.table", header = T, na.strings = "NA")
# the factors are not strings so gsub does not work on the whole df, gsub("chr", "", rec_rate)
rec_rate$map <- gsub('chr', '', rec_rate$map)
head(rec_rate)
str(rec_rate)

#get chr length from ~/Desktop/link_map_vanessa/output/qc_result/qualimap_dtol_210323/scaffold_length
chr_length <- read.table("window_based_data/scaffold_length", header = T)
colnames(chr_length) <- c("map", "chr_length")

#add the length of the previous chromosome so get the genomic position (for whole genome plot), use cumsum to get the position and then subtract with marker position later
...
write.table(rec_rate, "tables/rec_rate_markers.table")
