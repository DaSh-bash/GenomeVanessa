#Modified by Karin N??svall 210407
#packages needed polynom, splines, qualV, and plyr
...
#load inputs
load(file="all_maps_list_KN_rearr_mod.RData")
map.info.raw<-read.table("map_info_raw_KN_rearr_mod.txt", header=T, sep="\t")
...
#output to MareyMap "set" "map" "mkr" "phys" "gen" "vld"
marey_map <- all_maps_final$vcar[all_maps_final$vcar$good.marker==TRUE,c("sp","chr","marker","mb","cm")]
colnames(marey_map) <- c("set", "map", "mkr", "phys", "gen")
marey_map$phys <- marey_map$phys*1000000
head(marey_map)
write.table(marey_map,file="input_marey_map.txt")
...
write.table(file="map_info_final_rearr_mod.txt", map.info.final, row.names=F, sep="\t", quote=F)
