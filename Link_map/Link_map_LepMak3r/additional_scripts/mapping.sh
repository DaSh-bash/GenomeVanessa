...
do
awk '(NR==FNR){s[NR-1]=$0}(NR!=FNR){if ($1 in s) $3=s[$1];print}' $INPUT_PATH/snps.txt $INPUT_DIR/ordered.${lg}.distances > $OUTPUT_PATH/ordered.${lg}.mapped.txt
done
...
sed 's/\*//g' $OUTPUT_PATH/order_all.table > $OUTPUT_PATH/order_all2.table
