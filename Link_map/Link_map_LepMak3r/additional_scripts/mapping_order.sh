...
OUTPUT_PATH=$INPUT_PATH/mapped_trimmed
...
do
awk '(NR==FNR){s[NR-1]=$0}(NR!=FNR){if ($1 in s) $3=s[$1];print}' $INPUT_PATH/snps.txt $INPUT_DIR/ordered.${lg}.trimmed > $OUTPUT_PATH/ordered.${lg}.trimmed.txt
done
...
awk -v i=$i 'NR>3{print $1, $2, $3, $4, $5=i}' $OUTPUT_PATH/ordered.${i}.trimmed.txt | sed 's/HiC_scaffold_//g' >> $OUTPUT_PATH/order_all.txt
done
wait
