...
2. genes of interest: the output of BadiRate, genes belonging to gene families with different growth rate in Vanessa cardui compared the others (branch specific model higher fit then global model), specifically gene families with gene gains >0 in V. cardui. The gene gains have been parsed with separate scripts get_best_model_gains.sh and the the gene names in the orthogroups are retrieved with gene_selector_mod.sh
...
write.table(allRes_BP[1:50,], file = "../result_topGO/V_cardui_gains/res_topGO_VC_BP_50.txt")
write.table(allRes_CC[1:50,], file = "../result_topGO/V_cardui_gains/res_topGO_VC_CC_50.txt")
write.table(allRes_MF[1:50,], file = "../result_topGO/V_cardui_gains/res_topGO_VC_MF_50.txt")

write.table(rbind(allRes_BP[allRes_BP$p_adj<=0.05, ], allRes_CC[allRes_CC$p_adj<=0.05, ], allRes_MF[allRes_MF$p_adj<=0.05, ]), file = "../result_topGO/V_cardui_gains/res_topGO_VC_sign.txt")
...
```{r plots_fdr}
#use table with adjusted names (manually checked names in https://www.ebi.ac.uk/QuickGO/search/ to get full names of terms

allRes_all_fdr <- read.table(file = "../result_topGO/V_cardui_gains/res_topGO_VC_sign_mod.txt", header=T)
...
ggplot(allRes_all_fdr, aes(reorder(GO_descr, -Significant), Significant)) +
    geom_bar(stat = "identity", aes(fill=GO_class)) +
  facet_grid(~GO_class, scales = "free_x", space = "free_x") +
  ylab("Number of genes") +
  scale_fill_discrete(type=c("#d19200", "#916400","#4d3500"),guide = guide_legend(label.position = "top")) +
  theme(panel.background = element_blank(),
        strip.background = element_blank(),
        strip.text = element_blank(),
        axis.line = element_line(size = 0.2, colour = "grey"),
...
ggsave("../result_topGO/V_cardui_gains/barplot_brown.pdf")

ggsave("../result_topGO/V_cardui_gains/barplot_brown.png")
